    ...
    def rename_prefix(self):
        for subset in [self.train, self.query, self.gallery]:
            for i in range(len(subset)):
                items = list(subset[i])
                dirs = items[0].split('/')
                subset[i] = (osp.join(self.dataset_dir, *dirs[2:]), *items[1:])
        ...
        if not osp.exists(self.split_path):
            ...
            for _ in range(10):
                ...
                for pid, idx in enumerate(train_idxs):
                    ...
                    train.append((cam_a_img, pid + self.pid_begin, 0 + self.cam_id_begin, 0))
                    train.append((cam_b_img, pid + self.pid_begin, 1 + self.cam_id_begin, 0))
                ...
                for pid, idx in enumerate(test_idxs):
                    cam_a_img = cam_a_imgs[idx]
                    cam_b_img = cam_b_imgs[idx]
                    test_a.append((cam_a_img, pid, 0, 0))
                    test_b.append((cam_b_img, pid, 1, 0))
