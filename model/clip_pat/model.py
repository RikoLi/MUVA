...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int,
                attn_mask: torch.Tensor, hw_ratio=1, strict=True):
    ...
    model = CLIP(
        ...
        h_resolution, w_resolution, attn_mask
    )
    if vit:
        state_dict["visual.positional_embedding"] = resize_pos_embed(state_dict["visual.positional_embedding"], model.visual.positional_embedding, h_resolution, w_resolution, hw_ratio)
    else: #RN50
        state_dict["visual.attnpool.positional_embedding"] = resize_pos_embed(state_dict["visual.attnpool.positional_embedding"], model.visual.attnpool.positional_embedding, h_resolution, w_resolution, hw_ratio)
...
def resize_pos_embed(posemb, posemb_new, hight, width, hw_ratio=1):
    ...
    gs_old_h = int(math.sqrt(len(posemb_grid)*hw_ratio)) #14
